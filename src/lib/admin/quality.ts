...
export function checkQuality({ config, record, minWords, duplicateTitle, mdxProblems }: QualityInput): QualityWarning[] {
  const warnings: QualityWarning[] = []
  const bodies = (config.bodyFields ?? []).map((field) => ({ field, text: str(record[field]) }))
  const bodyText = bodies.map((b) => b.text).join('\n')

  if ((config.table === 'notes' || (config.table === 'pages' && record.template !== 'legal')) && words(bodyText) < minWords) {
    warnings.push({
      code: 'thin',
  ...
  if (hasSeoFields && !str(record.seo_description) && !str(record.summary) && !str(record.description)) {
    warnings.push({
      code: 'meta',
  ...
  if (seoTitle.length > 65) {
    warnings.push({ code: 'title-length', field: 'seo_title', message: `SEO title is ${seoTitle.length} characters; Google usually shows about 60.` })
  }
  ...
  if (/!\[\s*\]\(/.test(bodyText)) {
    warnings.push({ code: 'alt', message: 'An image has no alt text. Describe what it shows for screen readers and image search.' })
  }

  const linksInternally = /\]\(\/[^)]*\)|<RelatedLink\b/.test(bodyText)
  if ((config.table === 'notes' || config.table === 'pages' || config.table === 'assignments') && bodyText.trim() && !linksInternally) {
    warnings.push({
      code: 'links',
      message: 'No internal links. Link related weeks, notes or guides (e.g. <RelatedLink href="/qualifier/eligibility" />).',
    })
  }

  if (duplicateTitle) {
    warnings.push({ code: 'duplicate', field: config.titleColumn, message: 'Another item already uses this title. Titles should be unique.' })
  }
  ...
  if ((permission === 'permission_granted' || permission === 'official_link') && !str(record.source_url)) {
    warnings.push({ code: 'source', field: 'source_url', message: 'Add the source URL for shared or official material.' })
  }
  ...
  if (config.table === 'assignments' && record.type === 'graded') {
    ...
    if (release && new Date(release).getUTCFullYear() >= UNSCHEDULED_YEAR) {
      warnings.push({
        ...
        message: 'The solution release time is still the placeholder — walkthroughs will never unlock. Set it to after the deadline.',
      })
    }
    if (!str(record.due_at)) {
      warnings.push({ code: 'due', field: 'due_at', message: 'No due time set — the page and deadline widget cannot show it.' })
    }
  }

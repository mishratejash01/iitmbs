...
export async function GET(request: NextRequest) {
    ...
    await recordServerEvents([
      {
        name: 'login_failure',
        path: '/auth/callback',
        props: { provider: 'google', reason: oauthError ?? 'missing_code' },
      },
    ])
    return NextResponse.redirect(
      new URL(`/login?error=${oauthError === 'access_denied' ? 'denied' : 'callback'}`, url),
    )
  }
    ...
    await recordServerEvents([
      {
        name: 'login_failure',
        path: '/auth/callback',
        props: { provider: 'google', reason: error?.message ?? 'no_user' },
      },
    ])
  ...
  await recordServerEvents(
    [
      ...
      ...(firstLogin
        ? [{ name: 'signup_first_login', path: '/auth/callback', props: { provider: 'google' } }]
        : []),
    ],
    { userId: user.id },
  )

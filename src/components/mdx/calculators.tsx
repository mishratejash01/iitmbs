...
async function loadCalculatorData(): Promise<{ programs: CalculatorProgram[]; rules: QualifierRules } | null> {
  ...
  const calculatorPrograms = pages.flatMap((page) =>
    page && page.courses.length > 0
      ? [{ slug: page.program.slug, name: page.program.shortName, courses: page.courses.map((c) => c.shortName) }]
      : [],
  )

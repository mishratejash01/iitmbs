      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <select
            id="program_id"
            name="program_id"
            defaultValue={profile.program_id ?? programs[0]?.id}
            className={field}
          >
            {programs.map((p) => (
        ...
        <div>
          ...
          <select
            id="current_term"
            name="current_term"
            defaultValue={profile.current_term ?? terms[1]}
            className={field}
          >
            {terms.map((t) => (
    ...
    <form action={action} className="space-y-3">
      <label className="flex cursor-pointer items-start gap-3 rounded-control border border-border bg-card px-4 py-3 has-[:checked]:border-accent-strong has-[:checked]:bg-accent-soft">
        <input
          ...
          name="level"
          value="essential"
          defaultChecked={!detailed}
          className="mt-1 size-4 accent-[var(--accent-strong)]"
        />
        <span>
          <span className="block font-medium text-text">Essential only</span>
          <span className="block text-small text-muted">
            Pseudonymous page statistics. Nothing linked to your account.
          </span>
        </span>
      </label>
      <label className="flex cursor-pointer items-start gap-3 rounded-control border border-border bg-card px-4 py-3 has-[:checked]:border-accent-strong has-[:checked]:bg-accent-soft">
  ...
  return (
    <form action={action} className="space-y-3">
      <label htmlFor="confirm" className="block text-small text-text">
        Type <span className="font-mono font-semibold">DELETE</span> to permanently delete your
        account, bookmarks, history, progress and all analytics linked to you.
      </label>

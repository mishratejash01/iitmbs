 ...
 */
export function DeadlineWidget({ deadlines, limit = 6 }: { deadlines: Deadline[]; limit?: number }) {
  ...
  return (
    <section aria-labelledby="deadlines" className="rounded-card border border-border bg-card p-4 sm:p-5">
      ...
      <ul className="mt-3 divide-y divide-border">
        {visible.map((deadline) => (
          <li key={deadline.id}>
            <Link href={deadline.path} className="flex items-center justify-between gap-3 py-3 hover:text-accent-ink">
              <span className="min-w-0">
                <span className="block text-small font-medium text-text">
                  {deadline.courseShortName} · Week {deadline.weekNumber}
                </span>
                <span className="block text-xs text-muted">Due {formatDateTime(deadline.dueAt)}</span>
              </span>
              <span className="shrink-0 rounded-full bg-accent-soft px-2.5 py-0.5 text-xs font-medium text-accent-ink" suppressHydrationWarning>
                {now === null ? 'Due soon' : `in ${formatDuration((Date.parse(deadline.dueAt) - now) / 1000)}`}
              </span>
            </Link>
          </li>
        ))}

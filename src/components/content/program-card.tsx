...
export function ProgramCard({ data }: { data: ProgramPageData }) {
  ...
  return (
    <LinkCard href={program.path} className="flex h-full flex-col p-5 sm:p-6" data-track-area="card">
      ...
      {courses.length > 0 ? (
        <ul className="mt-4 flex flex-wrap gap-2" aria-label="Qualifier courses">
          {courses.map((course) => (
            <li key={course.id} className="rounded-full border border-border bg-surface px-3 py-1 text-xs font-medium text-text">
              {course.shortName}
            </li>
          ))}
        </ul>

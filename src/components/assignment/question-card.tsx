import { ChevronDown, Lightbulb, BookOpenCheck } from 'lucide-react'
  ...
  return (
    <article id={`q-${question.position}`} aria-labelledby={headingId} className="rounded-card border border-border bg-card p-4 sm:p-5">
      ...
      {question.conceptTags.length > 0 ? (
        <p className="mt-4 flex flex-wrap gap-1.5" aria-label="Concepts">
      ...
      {hint.content ? (
        <details
          className="group mt-4 rounded-control border border-border bg-surface"
      ...
      {released && (answer.content || explanation.content) ? (
        <details
          className="group mt-3 rounded-control border border-success/30 bg-success-soft"

-- ─────────────────────────────────────────────────────────────────────────────
-- Student notes: links to notes shared by students, grouped by course.
--
--   /notes                 index (title and SEO from the CMS page "notes")
--   /notes/<slug>          one course's notes
--
-- note_courses lists every course that has notes (any level, not only the
-- qualifier courses). Each note is a row in resources (kind 'link') pointing at
-- note_course_id, with the contributor's name for credit.
-- ─────────────────────────────────────────────────────────────────────────────

-- "/notes" is a fixed route, so no programme may use that slug.
...
create table public.note_courses (
  id uuid primary key default gen_random_uuid(),
  program_id uuid not null references public.programs (id) on delete restrict,
  -- Official course code, e.g. BSCS2007.
  code text not null check (code ~ '^[A-Z]{2,6}[0-9]{4}P?$'),
  ...
  -- Optional links to the qualifier course hub and the blog course guide.
...
create unique index note_courses_slug_key on public.note_courses (slug) where deleted_at is null;
create unique index note_courses_code_key on public.note_courses (code) where deleted_at is null;
create index note_courses_program_id_idx on public.note_courses (program_id);
create index note_courses_course_id_idx on public.note_courses (course_id);
create index note_courses_blog_post_id_idx on public.note_courses (blog_post_id);

create trigger note_courses_touch before insert or update on public.note_courses
  for each row execute function private.touch_row();
create trigger note_courses_publish before insert or update on public.note_courses
  for each row execute function private.default_published_at();

alter table public.note_courses enable row level security;
grant select on public.note_courses to anon, authenticated;
grant insert, update, delete on public.note_courses to authenticated;
grant all on public.note_courses to service_role;

create policy "note_courses: anon reads live rows" on public.note_courses for select to anon
  using (private.is_live(is_published, published_at, deleted_at));
create policy "note_courses: read live rows, staff read all" on public.note_courses for select to authenticated
  using (private.is_live(is_published, published_at, deleted_at) or (select private.is_staff()));
create policy "note_courses: staff insert" on public.note_courses for insert to authenticated
  with check ((select private.is_staff()));
create policy "note_courses: staff update" on public.note_courses for update to authenticated
  using ((select private.is_staff())) with check ((select private.is_staff()));
create policy "note_courses: admins delete" on public.note_courses for delete to authenticated
  using ((select private.is_admin()));

create trigger note_courses_audit after insert or update or delete on public.note_courses
  for each row execute function private.audit_row();
create trigger note_courses_revalidate after insert or update or delete on public.note_courses
  for each statement execute function private.notify_content_change();
create trigger note_courses_search_dirty after insert or update or delete on public.note_courses
  for each statement execute function private.mark_search_dirty();

-- Notes are resources that belong to a note course and credit a contributor.
alter table public.resources
  add column note_course_id uuid references public.note_courses (id) on delete set null,
  add column contributor text check (length(contributor) between 1 and 120);
create index resources_note_course_id_idx on public.resources (note_course_id, sort_order)
  where note_course_id is not null;

-- ── Routing view ─────────────────────────────────────────────────────────────
create or replace view private.live_note_courses
with (security_invoker = true)
as
select
  nc.*,
  '/notes/' || nc.slug as path,
  (select count(*) from public.resources r
   where r.note_course_id = nc.id
     and private.is_live(r.is_published, r.published_at, r.deleted_at)) as note_count,
  greatest(nc.updated_at, (select max(r.updated_at) from public.resources r
                           where r.note_course_id = nc.id
                             and private.is_live(r.is_published, r.published_at, r.deleted_at))) as content_updated_at
from public.note_courses nc
where private.is_live(nc.is_published, nc.published_at, nc.deleted_at);

grant select on private.live_note_courses to anon, authenticated, service_role;

-- ── Scheduled go-live and search refresh cover note courses too ─────────────
...
begin
  ...
  v_due := p_force or v_state.dirty or exists (
    select 1 from (
      ...
      select published_at from public.note_courses
    ) s
...
-- ── Sitemap: note course pages with at least one live note ──────────────────
...
-- ── Site search: add note course pages ──────────────────────────────────────
...
note_course_rows as (
  select
    ...
    lnc.name || ' ' || lnc.short_name || ' ' || lnc.code || ' notes pdf student notes',
    lnc.name || ' ' || lnc.short_name || ' ' || lnc.code || ' notes',
